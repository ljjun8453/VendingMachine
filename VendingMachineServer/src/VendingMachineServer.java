...
public class VendingMachineServer {
    ...
    public static void main(String[] args) {
        if (args.length < 2) {
            System.out.println("사용법: java VendingServer SERVER1 9001 [peerHost:peerPort] ...");
            return;
        }
        ...
        for (int i = 2; i < args.length; i++) {
            ...
            if (part.length == 2 && peerCount < peerHosts.length) {
                peerHosts[peerCount] = part[0];
                peerPorts[peerCount] = Integer.parseInt(part[1]);
                peerCount++;
            }
        }
    ...
    private static class ClientThread extends Thread {
        ...
        private String insertMoney(int money) {
            ...
            synchronized (VendingMachineServer.class) {
